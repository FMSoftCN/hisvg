...
typedef struct _HiSVGFilterPrimitiveOutput HiSVGFilterPrimitiveOutput;

struct _HiSVGFilterPrimitiveOutput {
    cairo_surface_t *surface;
    HiSVGIRect bounds;
};

typedef struct _HiSVGFilterContext HiSVGFilterContext;

struct _HiSVGFilterContext {
    gint width, height;
    HiSVGFilter *filter;
    GHashTable *results;
    cairo_surface_t *source_surface;
    cairo_surface_t *bg_surface;
    HiSVGFilterPrimitiveOutput lastresult;
...
typedef struct _HiSVGFilterPrimitive HiSVGFilterPrimitive;

/* We don't have real subclassing here.  If you derive something from
 * HiSVGFilterPrimitive, and don't need any special code to free your
 * HiSVGFilterPrimitiveFoo structure, you can just set
 *
 *    foo_filter->super.super.free = hisvg_filter_primitive_free;
 *
 * for the destructor.  Otherwise, create a custom destructor like this:
 *
 *    foo_filter->super.super.free = &hisvg_filter_primitive_foo_free;
 *
 *    ...
 *
 *    static void
 *    hisvg_filter_primitive_foo_free (HiSVGNode *self)
 *    {
 *        HiSVGFilterPrimitiveFoo *foo;
 *
 *        foo = (HiSVGFilterPrimitiveFoo *) self;
 *        g_free (foo->my_custom_stuff);
 *        g_free (foo->more_custom_stuff);
 *        ... etc ...
 *
 *        hisvg_filter_primitive_free (self);
 *    }
 *
 * That last call to hisvg_filter_primitive_free() will free the base HiSVGFilterPrimitive's own fields,
 * and your whole structure itself, via _hisvg_node_free().
 */
struct _HiSVGFilterPrimitive {
    HiSVGNode super;
    HiSVGLength x, y, width, height;
    GString *in;
    GString *result;

    void (*render) (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx);
};
...
static void
hisvg_filter_primitive_free (HiSVGNode *self)
{
    HiSVGFilterPrimitive *primitive;

    primitive = (HiSVGFilterPrimitive *) self;
...
static void
hisvg_filter_primitive_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    self->render (self, ctx);
}

static HiSVGIRect
hisvg_filter_primitive_get_bounds (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    HiSVGBbox box, otherbox;
    cairo_matrix_t affine;

    cairo_matrix_init_identity (&affine);
    hisvg_bbox_init (&box, &affine);
    hisvg_bbox_init (&otherbox, &ctx->affine);
    otherbox.virgin = 0;
    if (ctx->filter->filterunits == objectBoundingBox)
        _hisvg_push_view_box (ctx->ctx, 1., 1.);
    otherbox.rect.x = _hisvg_css_normalize_length (&ctx->filter->x, ctx->ctx, 'h');
    otherbox.rect.y = _hisvg_css_normalize_length (&ctx->filter->y, ctx->ctx, 'v');
    otherbox.rect.width = _hisvg_css_normalize_length (&ctx->filter->width, ctx->ctx, 'h');
    otherbox.rect.height = _hisvg_css_normalize_length (&ctx->filter->height, ctx->ctx, 'v');
    if (ctx->filter->filterunits == objectBoundingBox)
        _hisvg_pop_view_box (ctx->ctx);

    hisvg_bbox_insert (&box, &otherbox);

    if (self != NULL)
        if (self->x.factor != 'n' || self->y.factor != 'n' ||
            self->width.factor != 'n' || self->height.factor != 'n') {

            hisvg_bbox_init (&otherbox, &ctx->paffine);
            otherbox.virgin = 0;
            if (ctx->filter->primitiveunits == objectBoundingBox)
                _hisvg_push_view_box (ctx->ctx, 1., 1.);
            if (self->x.factor != 'n')
                otherbox.rect.x = _hisvg_css_normalize_length (&self->x, ctx->ctx, 'h');
            else
                otherbox.rect.x = 0;
            if (self->y.factor != 'n')
                otherbox.rect.y = _hisvg_css_normalize_length (&self->y, ctx->ctx, 'v');
            else
                otherbox.rect.y = 0;
            if (self->width.factor != 'n')
                otherbox.rect.width = _hisvg_css_normalize_length (&self->width, ctx->ctx, 'h');
            else
                otherbox.rect.width = ctx->ctx->vb.rect.width;
            if (self->height.factor != 'n')
                otherbox.rect.height = _hisvg_css_normalize_length (&self->height, ctx->ctx, 'v');
            else
                otherbox.rect.height = ctx->ctx->vb.rect.height;
            if (ctx->filter->primitiveunits == objectBoundingBox)
                _hisvg_pop_view_box (ctx->ctx);
            hisvg_bbox_clip (&box, &otherbox);
        }

    hisvg_bbox_init (&otherbox, &affine);
    otherbox.virgin = 0;
    otherbox.rect.x = 0;
    otherbox.rect.y = 0;
    ...
    hisvg_bbox_clip (&box, &otherbox);
    {
        HiSVGIRect output = { box.rect.x, box.rect.y,
...
_hisvg_image_surface_new (int width, int height)
{
    cairo_surface_t *surface;
...
static guchar
get_interp_pixel (guchar * src, gdouble ox, gdouble oy, guchar ch, HiSVGIRect boundarys,
                             guint rowstride)
{
    double xmod, ymod;
...
static void
hisvg_filter_fix_coordinate_system (HiSVGFilterContext * ctx, HiSVGState * state, HiSVGBbox *bbox)
{
...
static void
hisvg_alpha_blt (cairo_surface_t *src,
                gint srcx,
                gint srcy,
                gint srcwidth,
                gint srcheight,
...
static gboolean
hisvg_art_affine_image (cairo_surface_t *img, 
...
static void
hisvg_filter_free_pair (gpointer value)
{
    HiSVGFilterPrimitiveOutput *output;

    output = (HiSVGFilterPrimitiveOutput *) value;
    cairo_surface_destroy (output->surface);
    g_free (output);
}

static void
hisvg_filter_context_free (HiSVGFilterContext * ctx)
{
    if (!ctx)
	return;
 ...
 **/
cairo_surface_t *
hisvg_filter_render (HiSVGFilter *self,
                    cairo_surface_t *source,
                    HiSVGDrawingCtx *context, 
                    HiSVGBbox *bounds, 
                    char *channelmap)
{
    HiSVGFilterContext *ctx;
    HiSVGFilterPrimitive *current;
    guint i;
    ...
    ctx = g_new (HiSVGFilterContext, 1);
    ctx->filter = self;
    ctx->source_surface = source;
    ctx->bg_surface = NULL;
    ctx->results = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, hisvg_filter_free_pair);
    ctx->ctx = context;

    hisvg_filter_fix_coordinate_system (ctx, hisvg_current_state (context), bounds);

    ctx->lastresult.surface = cairo_surface_reference (source);
    ctx->lastresult.bounds = hisvg_filter_primitive_get_bounds (NULL, ctx);
    ...
    while(child)
    {
        current = (HiSVGFilterPrimitive*)HISVG_NODE_FROM_DOM_NODE (child);
        child = HISVG_DOM_ELEMENT_NODE_NEXT(child);
        if (HISVG_NODE_IS_FILTER_PRIMITIVE (&current->super))
            hisvg_filter_primitive_render (current, ctx);
    }
    ...
    hisvg_filter_context_free (ctx);

    return output;
}

/**
 * hisvg_filter_store_result:
 ...
 **/
static void
hisvg_filter_store_output (GString * name, HiSVGFilterPrimitiveOutput result, HiSVGFilterContext * ctx)
{
    HiSVGFilterPrimitiveOutput *store;

    cairo_surface_destroy (ctx->lastresult.surface);

    store = g_new (HiSVGFilterPrimitiveOutput, 1);
    *store = result;
...
static void
hisvg_filter_store_result (GString * name,
                          cairo_surface_t *surface,
                          HiSVGFilterContext * ctx)
{
    HiSVGFilterPrimitiveOutput output;
    ...
    hisvg_filter_store_output (name, output, ctx);
}
                   ...
                   HiSVGFilterContext * ctx)
{
    ...
    surface = _hisvg_image_surface_new (cairo_image_surface_get_width (source),
                                       cairo_image_surface_get_height (source));
    if (surface == NULL)
        return NULL;
...
static cairo_surface_t *
hisvg_compile_bg (HiSVGDrawingCtx * ctx)
{
    ...
    surface = _hisvg_image_surface_new (render->width, render->height);
    if (surface == NULL)
        return NULL;
 ...
 */
static cairo_surface_t *
hisvg_filter_get_bg (HiSVGFilterContext * ctx)
{
    if (!ctx->bg_surface)
        ctx->bg_surface = hisvg_compile_bg (ctx->ctx);
 ...
 **/
static HiSVGFilterPrimitiveOutput
hisvg_filter_get_result (GString * name, HiSVGFilterContext * ctx)
{
    HiSVGFilterPrimitiveOutput output;
    HiSVGFilterPrimitiveOutput *outputpointer;
    ...
    if (!strcmp (name->str, "SourceGraphic")) {
        ...
        output.surface = hisvg_filter_get_bg (ctx);
        if (output.surface)
        ...
        output.surface = surface_get_alpha (hisvg_filter_get_bg (ctx), ctx);
        return output;
    }

    outputpointer = (HiSVGFilterPrimitiveOutput *) (g_hash_table_lookup (ctx->results, name->str));
 ...
 */
static cairo_surface_t *
hisvg_filter_get_in (GString * name, HiSVGFilterContext * ctx)
{
    return hisvg_filter_get_result (name, ctx).surface;
}

static void
hisvg_filter_set_args (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilter *filter;

    filter = (HiSVGFilter *) self;

    if (hisvg_property_bag_size (atts)) {
        if ((value = hisvg_property_bag_lookup (atts, "filterUnits"))) {
            if (!strcmp (value, "userSpaceOnUse"))
                filter->filterunits = userSpaceOnUse;
            else
                filter->filterunits = objectBoundingBox;
        }
        if ((value = hisvg_property_bag_lookup (atts, "primitiveUnits"))) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "x")))
            filter->x = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "y")))
            filter->y = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "width")))
            filter->width = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "id"))) {
            HISVG_NODE_SET_ID(self, value);
            hisvg_defs_register_name (ctx->priv->defs, value, &filter->super);
        }
    }
}
 ...
 **/
HiSVGNode *
hisvg_new_filter (const char* name)
{
    HiSVGFilter *filter;

    filter = g_new (HiSVGFilter, 1);
    _hisvg_node_init (&filter->super, HISVG_NODE_TYPE_FILTER, name);
    filter->filterunits = objectBoundingBox;
    filter->primitiveunits = userSpaceOnUse;
    filter->x = _hisvg_css_parse_length ("-10%");
    filter->y = _hisvg_css_parse_length ("-10%");
    filter->width = _hisvg_css_parse_length ("120%");
    filter->height = _hisvg_css_parse_length ("120%");
    filter->super.set_atts = hisvg_filter_set_args;
    return (HiSVGNode *) filter;
}
...
typedef enum {
    ...
    difference
} HiSVGFilterPrimitiveBlendMode;

typedef struct _HiSVGFilterPrimitiveBlend HiSVGFilterPrimitiveBlend;
struct _HiSVGFilterPrimitiveBlend {
    HiSVGFilterPrimitive super;
    HiSVGFilterPrimitiveBlendMode mode;
    GString *in2;
};

static void
hisvg_filter_blend (HiSVGFilterPrimitiveBlendMode mode, 
                   cairo_surface_t *in, 
                   cairo_surface_t *in2,
                   cairo_surface_t* output, 
                   HiSVGIRect boundarys, 
                   int *channelmap)
{
    guchar i;
    gint x, y;
...
static void
hisvg_filter_primitive_blend_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    HiSVGIRect boundarys;

    HiSVGFilterPrimitiveBlend *upself;

    cairo_surface_t *output, *in, *in2;

    upself = (HiSVGFilterPrimitiveBlend *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);

    in = hisvg_filter_get_in (self->in, ctx);
    if (in == NULL)
      return;

    in2 = hisvg_filter_get_in (upself->in2, ctx);
    if (in2 == NULL) {
        cairo_surface_destroy (in);
        return;
    }

    output = _hisvg_image_surface_new (cairo_image_surface_get_width (in),
    ...
    hisvg_filter_blend (upself->mode, in, in2, output, boundarys, ctx->channelmap);

    hisvg_filter_store_result (self->result, output, ctx);
...
static void
hisvg_filter_primitive_blend_free (HiSVGNode * self)
{
    HiSVGFilterPrimitiveBlend *blend;
    blend = (HiSVGFilterPrimitiveBlend *) self;
...
static void
hisvg_filter_primitive_blend_set_atts (HiSVGNode * node, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveBlend *filter;

    filter = (HiSVGFilterPrimitiveBlend *) node;

    if (hisvg_property_bag_size (atts)) {
        if ((value = hisvg_property_bag_lookup (atts, "mode"))) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "in")))
            g_string_assign (filter->super.in, value);
        if ((value = hisvg_property_bag_lookup (atts, "in2")))
            g_string_assign (filter->in2, value);
        if ((value = hisvg_property_bag_lookup (atts, "result")))
            g_string_assign (filter->super.result, value);
        if ((value = hisvg_property_bag_lookup (atts, "x")))
            filter->super.x = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "y")))
            filter->super.y = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "width")))
            filter->super.width = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "id"))) {
            HISVG_NODE_SET_ID(node, value);
            hisvg_defs_register_name (ctx->priv->defs, value, &filter->super.super);
        }
    }
}

HiSVGNode *
hisvg_new_filter_primitive_blend (const char* name)
{
    HiSVGFilterPrimitiveBlend *filter;
    filter = g_new (HiSVGFilterPrimitiveBlend, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_BLEND, name);
    filter->mode = normal;
    ...
    filter->super.render = hisvg_filter_primitive_blend_render;
    filter->super.super.free = hisvg_filter_primitive_blend_free;
    filter->super.super.set_atts = hisvg_filter_primitive_blend_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveConvolveMatrix HiSVGFilterPrimitiveConvolveMatrix;

struct _HiSVGFilterPrimitiveConvolveMatrix {
    HiSVGFilterPrimitive super;
    double *KernelMatrix;
    double divisor;
    gint orderx, ordery;
...
static void
hisvg_filter_primitive_convolve_matrix_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar ch;
    gint x, y;
    gint i, j;
    ...
    HiSVGFilterPrimitiveConvolveMatrix *upself;
    ...
    upself = (HiSVGFilterPrimitiveConvolveMatrix *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
    ...
    output = _hisvg_image_surface_new (width, height);
    if (output == NULL) {
        cairo_surface_destroy (in);
        return;
    }
...
static void
hisvg_filter_primitive_convolve_matrix_free (HiSVGNode * self)
{
    HiSVGFilterPrimitiveConvolveMatrix *convolve;

    convolve = (HiSVGFilterPrimitiveConvolveMatrix *) self;
...
static void
hisvg_filter_primitive_convolve_matrix_set_atts (HiSVGNode * self,
                                                HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    gint i, j;
    guint listlen = 0;
    const char *value;
    gboolean has_target_x, has_target_y;
    HiSVGFilterPrimitiveConvolveMatrix *filter;

    filter = (HiSVGFilterPrimitiveConvolveMatrix *) self;
    has_target_x = 0;
    has_target_y = 0;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "targetX"))) {
            has_target_x = 1;
            filter->targetx = atoi (value);
        }
        if ((value = hisvg_property_bag_lookup (atts, "targetY"))) {
            has_target_y = 1;
            filter->targety = atoi (value);
        }
        if ((value = hisvg_property_bag_lookup (atts, "bias")))
            filter->bias = atof (value);
        if ((value = hisvg_property_bag_lookup (atts, "preserveAlpha"))) {
            if (!strcmp (value, "true"))
                filter->preservealpha = TRUE;
            else
                filter->preservealpha = FALSE;
        }
        if ((value = hisvg_property_bag_lookup (atts, "divisor")))
            filter->divisor = atof (value);
        if ((value = hisvg_property_bag_lookup (atts, "order"))) {
            double tempx, tempy;
            hisvg_css_parse_number_optional_number (value, &tempx, &tempy);
            filter->orderx = MAX (tempx, G_MAXINT);
            filter->ordery = MAX (tempy, G_MAXINT);
        }
        if ((value = hisvg_property_bag_lookup (atts, "kernelUnitLength")))
            hisvg_css_parse_number_optional_number (value, &filter->dx, &filter->dy);

        if ((value = hisvg_property_bag_lookup (atts, "kernelMatrix")))
            filter->KernelMatrix = hisvg_css_parse_number_list (value, &listlen);

        if ((value = hisvg_property_bag_lookup (atts, "edgeMode"))) {
...
HiSVGNode *
hisvg_new_filter_primitive_convolve_matrix (const char* name)
{
    HiSVGFilterPrimitiveConvolveMatrix *filter;
    filter = g_new (HiSVGFilterPrimitiveConvolveMatrix, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_CONVOLVE_MATRIX, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    ...
    filter->preservealpha = FALSE;
    filter->edgemode = 0;
    filter->super.render = hisvg_filter_primitive_convolve_matrix_render;
    filter->super.super.free = hisvg_filter_primitive_convolve_matrix_free;
    filter->super.super.set_atts = hisvg_filter_primitive_convolve_matrix_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveGaussianBlur
 HiSVGFilterPrimitiveGaussianBlur;

struct _HiSVGFilterPrimitiveGaussianBlur {
    HiSVGFilterPrimitive super;
    double sdx, sdy;
};
...
static void
hisvg_filter_primitive_gaussian_blur_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    HiSVGFilterPrimitiveGaussianBlur *upself;
    int width, height;
    cairo_surface_t *output, *in;
    HiSVGIRect boundarys;
    gfloat sdx, sdy;
    HiSVGFilterPrimitiveOutput op;
    cairo_t *cr;

    upself = (HiSVGFilterPrimitiveGaussianBlur *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);

    op = hisvg_filter_get_result (self->in, ctx);
    in = op.surface;
    ...
    op.surface = output;
    op.bounds = boundarys;
    hisvg_filter_store_output (self->result, op, ctx);
...
static void
hisvg_filter_primitive_gaussian_blur_set_atts (HiSVGNode * self,
                                              HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveGaussianBlur *filter;

    filter = (HiSVGFilterPrimitiveGaussianBlur *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "stdDeviation")))
            hisvg_css_parse_number_optional_number (value, &filter->sdx, &filter->sdy);
        if ((value = hisvg_property_bag_lookup (atts, "id"))) {
            HISVG_NODE_SET_ID(self, value);
            hisvg_defs_register_name (ctx->priv->defs, value, &filter->super.super);
        }
...
HiSVGNode *
hisvg_new_filter_primitive_gaussian_blur (const char* name)
{
    HiSVGFilterPrimitiveGaussianBlur *filter;
    filter = g_new (HiSVGFilterPrimitiveGaussianBlur, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_GAUSSIAN_BLUR, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    filter->sdx = 0;
    filter->sdy = 0;
    filter->super.render = hisvg_filter_primitive_gaussian_blur_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_gaussian_blur_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveOffset HiSVGFilterPrimitiveOffset;

struct _HiSVGFilterPrimitiveOffset {
    HiSVGFilterPrimitive super;
    HiSVGLength dx, dy;
};

static void
hisvg_filter_primitive_offset_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar ch;
    gint x, y;
    gint rowstride, height, width;
    ...
    HiSVGFilterPrimitiveOutput out;
    HiSVGFilterPrimitiveOffset *upself;
    ...
    upself = (HiSVGFilterPrimitiveOffset *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
    ...
    dx = _hisvg_css_normalize_length (&upself->dx, ctx->ctx, 'w');
    dy = _hisvg_css_normalize_length (&upself->dy, ctx->ctx, 'v');
    ...
    hisvg_filter_store_output (self->result, out, ctx);
...
static void
hisvg_filter_primitive_offset_set_atts (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveOffset *filter;

    filter = (HiSVGFilterPrimitiveOffset *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "dx")))
            filter->dx = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "dy")))
            filter->dy = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "id"))) {
            HISVG_NODE_SET_ID(self, value);
            hisvg_defs_register_name (ctx->priv->defs, value, &filter->super.super);
        }
...
HiSVGNode *
hisvg_new_filter_primitive_offset (const char* name)
{
    HiSVGFilterPrimitiveOffset *filter;
    filter = g_new (HiSVGFilterPrimitiveOffset, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_OFFSET, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    filter->dy = _hisvg_css_parse_length ("0");
    filter->dx = _hisvg_css_parse_length ("0");
    filter->super.render = hisvg_filter_primitive_offset_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_offset_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveMerge HiSVGFilterPrimitiveMerge;

struct _HiSVGFilterPrimitiveMerge {
    HiSVGFilterPrimitive super;
};

static void
hisvg_filter_primitive_merge_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guint i;
    HiSVGIRect boundarys;

    HiSVGFilterPrimitiveMerge *upself;

    cairo_surface_t *output, *in;

    upself = (HiSVGFilterPrimitiveMerge *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);

    output = _hisvg_image_surface_new (ctx->width, ctx->height);
    if (output == NULL) {
        return;
    }

    HLDomElementNode* child = HISVG_DOM_ELEMENT_NODE_FIRST_CHILD(upself->super.super.base);
    while(child)
    {
        HiSVGFilterPrimitive* mn  = (HiSVGFilterPrimitive*)HISVG_NODE_FROM_DOM_NODE (child);
        child = HISVG_DOM_ELEMENT_NODE_NEXT(child);
        if (HISVG_NODE_TYPE (&mn->super) != HISVG_NODE_TYPE_FILTER_PRIMITIVE_MERGE_NODE)
            continue;
        in = hisvg_filter_get_in (mn->in, ctx);
        if (in == NULL)
            continue;

        hisvg_alpha_blt (in, boundarys.x0, boundarys.y0, boundarys.x1 - boundarys.x0,
...
static void
hisvg_filter_primitive_merge_free (HiSVGNode * self)
{
    HiSVGFilterPrimitiveMerge *merge;

    merge = (HiSVGFilterPrimitiveMerge *) self;
...
static void
hisvg_filter_primitive_merge_set_atts (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveMerge *filter;

    filter = (HiSVGFilterPrimitiveMerge *) self;
...
HiSVGNode *
hisvg_new_filter_primitive_merge (const char* name)
{
    HiSVGFilterPrimitiveMerge *filter;
    filter = g_new (HiSVGFilterPrimitiveMerge, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_MERGE, name);
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    filter->super.render = hisvg_filter_primitive_merge_render;
    filter->super.super.free = hisvg_filter_primitive_merge_free;

    filter->super.super.set_atts = hisvg_filter_primitive_merge_set_atts;
    return (HiSVGNode *) filter;
}

static void
hisvg_filter_primitive_merge_node_set_atts (HiSVGNode * self,
                                           HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "in")))
            g_string_assign (((HiSVGFilterPrimitive *) self)->in, value);
    }
}

static void
hisvg_filter_primitive_merge_node_free (HiSVGNode * self)
{
    HiSVGFilterPrimitive *merge_node;

    merge_node = (HiSVGFilterPrimitive *) self;
...
static void
hisvg_filter_primitive_merge_node_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    /* todo */
}

HiSVGNode *
hisvg_new_filter_primitive_merge_node (const char* name)
{
    HiSVGFilterPrimitive *filter;
    filter = g_new (HiSVGFilterPrimitive, 1);
    _hisvg_node_init (&filter->super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_MERGE_NODE, name);
    filter->in = g_string_new ("none");
    filter->super.free = hisvg_filter_primitive_merge_node_free;
    filter->render = hisvg_filter_primitive_merge_node_render;
    filter->super.set_atts = hisvg_filter_primitive_merge_node_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveColorMatrix
 HiSVGFilterPrimitiveColorMatrix;

struct _HiSVGFilterPrimitiveColorMatrix {
    HiSVGFilterPrimitive super;
    gint *KernelMatrix;
};

static void
hisvg_filter_primitive_color_matrix_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar ch;
    gint x, y;
    gint i;
    ...
    HiSVGFilterPrimitiveColorMatrix *upself;

    cairo_surface_t *output, *in;

    int sum;

    upself = (HiSVGFilterPrimitiveColorMatrix *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
...
static void
hisvg_filter_primitive_color_matrix_free (HiSVGNode * self)
{
    HiSVGFilterPrimitiveColorMatrix *matrix;

    matrix = (HiSVGFilterPrimitiveColorMatrix *) self;
...
static void
hisvg_filter_primitive_color_matrix_set_atts (HiSVGNode * self, HiSVGHandle * ctx,
                                              HiSVGPropertyBag * atts)
{
    gint type;
    guint listlen = 0;
    const char *value;
    HiSVGFilterPrimitiveColorMatrix *filter;

    filter = (HiSVGFilterPrimitiveColorMatrix *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "values"))) {
            unsigned int i;
            double *temp = hisvg_css_parse_number_list (value, &listlen);
            filter->KernelMatrix = g_new (int, listlen);
            for (i = 0; i < listlen; i++)
                filter->KernelMatrix[i] = temp[i] * 255.;
            g_free (temp);
        }
        if ((value = hisvg_property_bag_lookup (atts, "type"))) {
...
HiSVGNode *
hisvg_new_filter_primitive_color_matrix (const char* name)
{
    HiSVGFilterPrimitiveColorMatrix *filter;
    filter = g_new (HiSVGFilterPrimitiveColorMatrix, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_COLOR_MATRIX, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    filter->KernelMatrix = NULL;
    filter->super.render = hisvg_filter_primitive_color_matrix_render;
    filter->super.super.free = hisvg_filter_primitive_color_matrix_free;

    filter->super.super.set_atts = hisvg_filter_primitive_color_matrix_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGNodeComponentTransferFunc HiSVGNodeComponentTransferFunc;

typedef gint (*ComponentTransferFunc) (gint C, HiSVGNodeComponentTransferFunc * user_data);

typedef struct _HiSVGFilterPrimitiveComponentTransfer
 HiSVGFilterPrimitiveComponentTransfer;

struct _HiSVGNodeComponentTransferFunc {
    HiSVGNode super;
...
struct _HiSVGFilterPrimitiveComponentTransfer {
    HiSVGFilterPrimitive super;
};

static gint
identity_component_transfer_func (gint C, HiSVGNodeComponentTransferFunc * user_data)
{
    return C;
}

static gint
table_component_transfer_func (gint C, HiSVGNodeComponentTransferFunc * user_data)
{
    guint k;
...
static gint
discrete_component_transfer_func (gint C, HiSVGNodeComponentTransferFunc * user_data)
{
    gint k;
...
static gint
linear_component_transfer_func (gint C, HiSVGNodeComponentTransferFunc * user_data)
{
...
static gint
gamma_component_transfer_func (gint C, HiSVGNodeComponentTransferFunc * user_data)
{
...
static void
hisvg_filter_primitive_component_transfer_render (HiSVGFilterPrimitive *
                                                 self, HiSVGFilterContext * ctx)
{
    gint x, y, c;
    gint rowstride, height, width;
    HiSVGIRect boundarys;
    HiSVGNodeComponentTransferFunc *channels[4];
    ...
    for (c = 0; c < 4; c++) {
        char channel = "rgba"[c]; /* see hisvg_standard_element_start() for where these chars come from */
        HLDomElementNode* child = HISVG_DOM_ELEMENT_NODE_FIRST_CHILD(self->super.base);
        while(child)
        {
            HiSVGNode *child_node = HISVG_NODE_FROM_DOM_NODE (child);
            child = HISVG_DOM_ELEMENT_NODE_NEXT(child);
            if (HISVG_NODE_TYPE (child_node) == HISVG_NODE_TYPE_COMPONENT_TRANFER_FUNCTION) {
                HiSVGNodeComponentTransferFunc *temp = (HiSVGNodeComponentTransferFunc *) child_node;
...
static void
hisvg_filter_primitive_component_transfer_set_atts (HiSVGNode * self, HiSVGHandle * ctx,
                                                   HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveComponentTransfer *filter;

    filter = (HiSVGFilterPrimitiveComponentTransfer *) self;
...
HiSVGNode *
hisvg_new_filter_primitive_component_transfer (const char* name)
{
    HiSVGFilterPrimitiveComponentTransfer *filter;

    filter = g_new (HiSVGFilterPrimitiveComponentTransfer, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_COMPONENT_TRANSFER, name);
    filter->super.result = g_string_new ("none");
    filter->super.in = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    filter->super.render = hisvg_filter_primitive_component_transfer_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_component_transfer_set_atts;
    return (HiSVGNode *) filter;
}

static void
hisvg_node_component_transfer_function_set_atts (HiSVGNode * self,
                                                HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGNodeComponentTransferFunc *data = (HiSVGNodeComponentTransferFunc *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "tableValues"))) {
            unsigned int i;
            double *temp = hisvg_css_parse_number_list (value,
                                                       &data->nbTableValues);
        ...
        if ((value = hisvg_property_bag_lookup (atts, "slope"))) {
            data->slope = g_ascii_strtod (value, NULL) * 255.;
        }
        if ((value = hisvg_property_bag_lookup (atts, "intercept"))) {
            data->intercept = g_ascii_strtod (value, NULL) * 255.;
        }
        if ((value = hisvg_property_bag_lookup (atts, "amplitude"))) {
            data->amplitude = g_ascii_strtod (value, NULL) * 255.;
        }
        if ((value = hisvg_property_bag_lookup (atts, "exponent"))) {
...
static void
hisvg_component_transfer_function_free (HiSVGNode * self)
{
    HiSVGNodeComponentTransferFunc *filter = (HiSVGNodeComponentTransferFunc *) self;
...
HiSVGNode *
hisvg_new_node_component_transfer_function (const char* name, char channel)
{
    HiSVGNodeComponentTransferFunc *filter;

    filter = g_new0 (HiSVGNodeComponentTransferFunc, 1);
    _hisvg_node_init (&filter->super, HISVG_NODE_TYPE_COMPONENT_TRANFER_FUNCTION, name);
    filter->super.free = hisvg_component_transfer_function_free;
    filter->super.set_atts = hisvg_node_component_transfer_function_set_atts;
...
typedef struct _HiSVGFilterPrimitiveErode
 HiSVGFilterPrimitiveErode;

struct _HiSVGFilterPrimitiveErode {
    HiSVGFilterPrimitive super;
    double rx, ry;
    int mode;
};
...
static void
hisvg_filter_primitive_erode_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar ch, extreme;
    gint x, y;
    gint i, j;
    ...
    HiSVGFilterPrimitiveErode *upself;
    ...
    upself = (HiSVGFilterPrimitiveErode *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
...
static void
hisvg_filter_primitive_erode_set_atts (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveErode *filter;

    filter = (HiSVGFilterPrimitiveErode *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "radius"))) {
            hisvg_css_parse_number_optional_number (value, &filter->rx, &filter->ry);
        }
        if ((value = hisvg_property_bag_lookup (atts, "operator"))) {
...
HiSVGNode *
hisvg_new_filter_primitive_erode (const char* name)
{
    HiSVGFilterPrimitiveErode *filter;
    filter = g_new (HiSVGFilterPrimitiveErode, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_ERODE, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    ...
    filter->ry = 0;
    filter->mode = 0;
    filter->super.render = hisvg_filter_primitive_erode_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_erode_set_atts;
    return (HiSVGNode *) filter;
}
...
} HiSVGFilterPrimitiveCompositeMode;

typedef struct _HiSVGFilterPrimitiveComposite HiSVGFilterPrimitiveComposite;
struct _HiSVGFilterPrimitiveComposite {
    HiSVGFilterPrimitive super;
    HiSVGFilterPrimitiveCompositeMode mode;
    GString *in2;
...
static void
hisvg_filter_primitive_composite_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar i;
    gint x, y;
    gint rowstride, height, width;
    ...
    HiSVGFilterPrimitiveComposite *upself;

    cairo_surface_t *output, *in, *in2;

    upself = (HiSVGFilterPrimitiveComposite *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
...
static void
hisvg_filter_primitive_composite_free (HiSVGNode * self)
{
    HiSVGFilterPrimitiveComposite *composite;

    composite = (HiSVGFilterPrimitiveComposite *) self;
...
static void
hisvg_filter_primitive_composite_set_atts (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveComposite *filter;

    filter = (HiSVGFilterPrimitiveComposite *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "k1")))
            filter->k1 = g_ascii_strtod (value, NULL) * 255.;
        if ((value = hisvg_property_bag_lookup (atts, "k2")))
            filter->k2 = g_ascii_strtod (value, NULL) * 255.;
        if ((value = hisvg_property_bag_lookup (atts, "k3")))
            filter->k3 = g_ascii_strtod (value, NULL) * 255.;
        if ((value = hisvg_property_bag_lookup (atts, "k4")))
...
HiSVGNode *
hisvg_new_filter_primitive_composite (const char* name)
{
    HiSVGFilterPrimitiveComposite *filter;
    filter = g_new (HiSVGFilterPrimitiveComposite, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_COMPOSITE, name);
    filter->mode = COMPOSITE_MODE_OVER;
    filter->super.in = g_string_new ("none");
    filter->in2 = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    ...
    filter->k3 = 0;
    filter->k4 = 0;
    filter->super.render = hisvg_filter_primitive_composite_render;
    filter->super.super.free = hisvg_filter_primitive_composite_free;
    filter->super.super.set_atts = hisvg_filter_primitive_composite_set_atts;
    return (HiSVGNode *) filter;
}
...
static void
hisvg_filter_primitive_flood_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar i;
    gint x, y;
    gint rowstride, height, width;
...
static void
hisvg_filter_primitive_flood_set_atts (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value, *id = NULL;
    HiSVGFilterPrimitive *filter = (HiSVGFilterPrimitive *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "id"))) {
            HISVG_NODE_SET_ID(self, value);
            hisvg_defs_register_name (ctx->priv->defs, id = value, &filter->super);
        }
        hisvg_parse_style_attrs (ctx, self->state, "feFlood", NULL, id, atts);
    }
}

HiSVGNode *
hisvg_new_filter_primitive_flood (const char* name)
{
    HiSVGFilterPrimitive *filter;
    filter = g_new (HiSVGFilterPrimitive, 1);
    _hisvg_node_init (&filter->super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_FLOOD, name);
    filter->in = g_string_new ("none");
    filter->result = g_string_new ("none");
    filter->x.factor = filter->y.factor = filter->width.factor = filter->height.factor = 'n';
    filter->render = hisvg_filter_primitive_flood_render;
    filter->super.free = hisvg_filter_primitive_free;
    filter->super.set_atts = hisvg_filter_primitive_flood_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveDisplacementMap HiSVGFilterPrimitiveDisplacementMap;

struct _HiSVGFilterPrimitiveDisplacementMap {
    HiSVGFilterPrimitive super;
    gint dx, dy;
...
static void
hisvg_filter_primitive_displacement_map_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar ch, xch, ych;
    gint x, y;
    gint rowstride, height, width;
    ...
    HiSVGFilterPrimitiveDisplacementMap *upself;

    cairo_surface_t *output, *in, *in2;

    double ox, oy;

    upself = (HiSVGFilterPrimitiveDisplacementMap *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
...
static void
hisvg_filter_primitive_displacement_map_free (HiSVGNode * self)
{
    HiSVGFilterPrimitiveDisplacementMap *dmap;

    dmap = (HiSVGFilterPrimitiveDisplacementMap *) self;
...
static void
hisvg_filter_primitive_displacement_map_set_atts (HiSVGNode * self, HiSVGHandle * ctx,
                                                 HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveDisplacementMap *filter;

    filter = (HiSVGFilterPrimitiveDisplacementMap *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "xChannelSelector")))
            filter->xChannelSelector = (value)[0];
        if ((value = hisvg_property_bag_lookup (atts, "yChannelSelector")))
            filter->yChannelSelector = (value)[0];
        if ((value = hisvg_property_bag_lookup (atts, "scale")))
...
HiSVGNode *
hisvg_new_filter_primitive_displacement_map (const char* name)
{
    HiSVGFilterPrimitiveDisplacementMap *filter;
    filter = g_new (HiSVGFilterPrimitiveDisplacementMap, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_DISPLACEMENT_MAP, name);
    filter->super.in = g_string_new ("none");
    filter->in2 = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
    ...
    filter->scale = 0;
    filter->super.render = hisvg_filter_primitive_displacement_map_render;
    filter->super.super.free = hisvg_filter_primitive_displacement_map_free;
    filter->super.super.set_atts = hisvg_filter_primitive_displacement_map_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveTurbulence HiSVGFilterPrimitiveTurbulence;
struct _HiSVGFilterPrimitiveTurbulence {
    HiSVGFilterPrimitive super;
...
static void
feTurbulence_init (HiSVGFilterPrimitiveTurbulence * filter)
{
    double s;
    int i, j, k, lSeed;
...
static double
feTurbulence_noise2 (HiSVGFilterPrimitiveTurbulence * filter,
...
static double
feTurbulence_turbulence (HiSVGFilterPrimitiveTurbulence * filter,
...
static void
hisvg_filter_primitive_turbulence_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    HiSVGFilterPrimitiveTurbulence *upself;
    ...
    upself = (HiSVGFilterPrimitiveTurbulence *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
...
static void
hisvg_filter_primitive_turbulence_set_atts (HiSVGNode * self, HiSVGHandle * ctx,
                                           HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveTurbulence *filter;

    filter = (HiSVGFilterPrimitiveTurbulence *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "height")))
            filter->super.height = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "baseFrequency")))
            hisvg_css_parse_number_optional_number (value, &filter->fBaseFreqX, &filter->fBaseFreqY);
        if ((value = hisvg_property_bag_lookup (atts, "numOctaves")))
            filter->nNumOctaves = atoi (value);
        if ((value = hisvg_property_bag_lookup (atts, "seed")))
            filter->seed = atoi (value);
        if ((value = hisvg_property_bag_lookup (atts, "stitchTiles")))
...
HiSVGNode *
hisvg_new_filter_primitive_turbulence (const char* name)
{
    HiSVGFilterPrimitiveTurbulence *filter;
    filter = g_new (HiSVGFilterPrimitiveTurbulence, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_TURBULENCE, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    ...
    filter->super.render = hisvg_filter_primitive_turbulence_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_turbulence_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveImage HiSVGFilterPrimitiveImage;

struct _HiSVGFilterPrimitiveImage {
    HiSVGFilterPrimitive super;
    HiSVGHandle *ctx;
    GString *href;
};
...
static cairo_surface_t *
hisvg_filter_primitive_image_render_in (HiSVGFilterPrimitive * self, HiSVGFilterContext * context)
{
    HiSVGDrawingCtx *ctx;
    HiSVGFilterPrimitiveImage *upself;
    HiSVGNode *drawable;
    cairo_surface_t *result;

    ctx = context->ctx;

    upself = (HiSVGFilterPrimitiveImage *) self;

    if (!upself->href)
        return NULL;

    drawable = hisvg_acquire_node (ctx, upself->href->str);
    if (!drawable)
        return NULL;

    hisvg_current_state (ctx)->affine = context->paffine;

    result = hisvg_get_surface_of_node (ctx, drawable, context->width, context->height);

    hisvg_release_node (ctx, drawable);

    return result;
}

static cairo_surface_t *
hisvg_filter_primitive_image_render_ext (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    HiSVGIRect boundarys;
    HiSVGFilterPrimitiveImage *upself;
    ...
    img = hisvg_cairo_surface_new_from_href (upself->ctx,
                                            upself->href->str,
                                            NULL);
    if (!img)
        return NULL;
    ...
    if (cairo_surface_status (intermediate) != CAIRO_STATUS_SUCCESS ||
        !hisvg_art_affine_image (img, intermediate,
                                &ctx->paffine,
...
static void
hisvg_filter_primitive_image_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    HiSVGIRect boundarys;
    HiSVGFilterPrimitiveImage *upself;
    HiSVGFilterPrimitiveOutput op;
    ...
    img = hisvg_filter_primitive_image_render_in (self, ctx);
    if (img == NULL) {
        img = hisvg_filter_primitive_image_render_ext (self, ctx);
        x = y = 0;
    } else {
        x = boundarys.x0;
        y = boundarys.y0;
...
static void
hisvg_filter_primitive_image_free (HiSVGNode * self)
{
    HiSVGFilterPrimitiveImage *image;

    image = (HiSVGFilterPrimitiveImage *) self;
...
static void
hisvg_filter_primitive_image_set_atts (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveImage *filter;

    filter = (HiSVGFilterPrimitiveImage *) self;
    filter->ctx = ctx;
...
HiSVGNode *
hisvg_new_filter_primitive_image (const char* name)
{
    HiSVGFilterPrimitiveImage *filter;
    filter = g_new (HiSVGFilterPrimitiveImage, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_IMAGE, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    filter->super.render = hisvg_filter_primitive_image_render;
    filter->super.super.free = hisvg_filter_primitive_image_free;
    filter->super.super.set_atts = hisvg_filter_primitive_image_set_atts;
    filter->href = NULL;
    return (HiSVGNode *) filter;
}
...
static vector3
get_surface_normal (guchar * I, HiSVGIRect boundarys, gint x, gint y,
...
typedef struct _HiSVGNodeLightSource HiSVGNodeLightSource;

struct _HiSVGNodeLightSource {
    HiSVGNode super;
    lightType type;
    gdouble azimuth;
    gdouble elevation;
    HiSVGLength x, y, z, pointsAtX, pointsAtY, pointsAtZ;
    gdouble specularExponent;
    gdouble limitingconeAngle;
};

static vector3
get_light_direction (HiSVGNodeLightSource * source, gdouble x1, gdouble y1, gdouble z,
                     cairo_matrix_t *affine, HiSVGDrawingCtx * ctx)
{
    vector3 output;
    ...
    default:
        {
            ...
            output.x = _hisvg_css_normalize_length (&source->x, ctx, 'h') - x;
            output.y = _hisvg_css_normalize_length (&source->y, ctx, 'v') - y;
            output.z = _hisvg_css_normalize_length (&source->z, ctx, 'o') - z;
            output = normalise (output);
        }
        break;
    }
...
static vector3
get_light_color (HiSVGNodeLightSource * source, vector3 color,
                  gdouble x1, gdouble y1, gdouble z, cairo_matrix_t *affine, HiSVGDrawingCtx * ctx)
{
    ...
    sx = _hisvg_css_normalize_length (&source->x, ctx, 'h');
    sy = _hisvg_css_normalize_length (&source->y, ctx, 'v');
    sz = _hisvg_css_normalize_length (&source->z, ctx, 'o');
    spx = _hisvg_css_normalize_length (&source->pointsAtX, ctx, 'h');
    spy = _hisvg_css_normalize_length (&source->pointsAtY, ctx, 'v');
    spz = _hisvg_css_normalize_length (&source->pointsAtZ, ctx, 'o');
...
static void
hisvg_node_light_source_set_atts (HiSVGNode * self,
                                 HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    HiSVGNodeLightSource *data;
    const char *value;

    data = (HiSVGNodeLightSource *) self;

    if (hisvg_property_bag_size (atts)) {
        if ((value = hisvg_property_bag_lookup (atts, "azimuth")))
            data->azimuth = hisvg_css_parse_angle (value) / 180.0 * M_PI;
        if ((value = hisvg_property_bag_lookup (atts, "elevation")))
            data->elevation = hisvg_css_parse_angle (value) / 180.0 * M_PI;
        if ((value = hisvg_property_bag_lookup (atts, "limitingConeAngle")))
            data->limitingconeAngle = hisvg_css_parse_angle (value) / 180.0 * M_PI;
        if ((value = hisvg_property_bag_lookup (atts, "x")))
            data->x = data->pointsAtX = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "y")))
            data->y = data->pointsAtX = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "z")))
            data->z = data->pointsAtX = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "pointsAtX")))
            data->pointsAtX = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "pointsAtY")))
            data->pointsAtY = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "pointsAtZ")))
            data->pointsAtZ = _hisvg_css_parse_length (value);
        if ((value = hisvg_property_bag_lookup (atts, "specularExponent")))
            data->specularExponent = g_ascii_strtod (value, NULL);
    }
}

HiSVGNode *
hisvg_new_node_light_source (const char* name, char type)
{
    HiSVGNodeLightSource *data;
    data = g_new (HiSVGNodeLightSource, 1);
    _hisvg_node_init (&data->super, HISVG_NODE_TYPE_LIGHT_SOURCE, name);
    data->super.free = _hisvg_node_free;
    data->super.set_atts = hisvg_node_light_source_set_atts;
...
typedef struct _HiSVGFilterPrimitiveDiffuseLighting HiSVGFilterPrimitiveDiffuseLighting;

struct _HiSVGFilterPrimitiveDiffuseLighting {
    HiSVGFilterPrimitive super;
    gdouble dx, dy;
...
static void
hisvg_filter_primitive_diffuse_lighting_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    gint x, y;
    ...
    HiSVGNodeLightSource *source = NULL;
    HiSVGIRect boundarys;

    guchar *in_pixels;
    guchar *output_pixels;

    HiSVGFilterPrimitiveDiffuseLighting *upself;
    ...
    while(child)
    {
        HiSVGNode* temp = HISVG_NODE_FROM_DOM_NODE (child);
        child = HISVG_DOM_ELEMENT_NODE_NEXT(child);
        if (HISVG_NODE_TYPE (temp) == HISVG_NODE_TYPE_LIGHT_SOURCE) {
            source = (HiSVGNodeLightSource *) temp;
        }
    }
    if (source == NULL)
        return;
    ...
    upself = (HiSVGFilterPrimitiveDiffuseLighting *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
...
static void
hisvg_filter_primitive_diffuse_lighting_set_atts (HiSVGNode * self, HiSVGHandle * ctx,
                                                 HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveDiffuseLighting *filter;

    filter = (HiSVGFilterPrimitiveDiffuseLighting *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "kernelUnitLength")))
            hisvg_css_parse_number_optional_number (value, &filter->dx, &filter->dy);
        if ((value = hisvg_property_bag_lookup (atts, "lighting-color")))
            filter->lightingcolor = hisvg_css_parse_color (value, 0);
        if ((value = hisvg_property_bag_lookup (atts, "diffuseConstant")))
            filter->diffuseConstant = g_ascii_strtod (value, NULL);
        if ((value = hisvg_property_bag_lookup (atts, "surfaceScale")))
...
HiSVGNode *
hisvg_new_filter_primitive_diffuse_lighting (const char* name)
{
    HiSVGFilterPrimitiveDiffuseLighting *filter;
    filter = g_new (HiSVGFilterPrimitiveDiffuseLighting, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_DIFFUSE_LIGHTING, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    ...
    filter->super.render = hisvg_filter_primitive_diffuse_lighting_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_diffuse_lighting_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveSpecularLighting HiSVGFilterPrimitiveSpecularLighting;

struct _HiSVGFilterPrimitiveSpecularLighting {
    HiSVGFilterPrimitive super;
...
static void
hisvg_filter_primitive_specular_lighting_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    gint x, y;
    ...
    HiSVGFilterPrimitiveSpecularLighting *upself;
    ...
    upself = (HiSVGFilterPrimitiveSpecularLighting *) self;
    boundarys = hisvg_filter_primitive_get_bounds (self, ctx);
...
static void
hisvg_filter_primitive_specular_lighting_set_atts (HiSVGNode * self, HiSVGHandle * ctx,
                                                  HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveSpecularLighting *filter;

    filter = (HiSVGFilterPrimitiveSpecularLighting *) self;
    ...
    if (hisvg_property_bag_size (atts)) {
        ...
        if ((value = hisvg_property_bag_lookup (atts, "lighting-color")))
            filter->lightingcolor = hisvg_css_parse_color (value, 0);
        if ((value = hisvg_property_bag_lookup (atts, "specularConstant")))
...
HiSVGNode *
hisvg_new_filter_primitive_specular_lighting (const char* name)
{
    HiSVGFilterPrimitiveSpecularLighting *filter;
    filter = g_new (HiSVGFilterPrimitiveSpecularLighting, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_SPECULAR_LIGHTING, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    ...
    filter->lightingcolor = 0xFFFFFFFF;
    filter->super.render = hisvg_filter_primitive_specular_lighting_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_specular_lighting_set_atts;
    return (HiSVGNode *) filter;
}
...
typedef struct _HiSVGFilterPrimitiveTile
 HiSVGFilterPrimitiveTile;

struct _HiSVGFilterPrimitiveTile {
    HiSVGFilterPrimitive super;
};
...
static void
hisvg_filter_primitive_tile_render (HiSVGFilterPrimitive * self, HiSVGFilterContext * ctx)
{
    guchar i;
    gint x, y, rowstride;
    HiSVGIRect boundarys, oboundarys;

    HiSVGFilterPrimitiveOutput input;
    ...
    oboundarys = hisvg_filter_primitive_get_bounds (self, ctx);

    input = hisvg_filter_get_result (self->in, ctx);
    in = input.surface;
...
static void
hisvg_filter_primitive_tile_set_atts (HiSVGNode * self, HiSVGHandle * ctx, HiSVGPropertyBag * atts)
{
    const char *value;
    HiSVGFilterPrimitiveTile *filter;

    filter = (HiSVGFilterPrimitiveTile *) self;
...
HiSVGNode *
hisvg_new_filter_primitive_tile (const char* name)
{
    HiSVGFilterPrimitiveTile *filter;
    filter = g_new (HiSVGFilterPrimitiveTile, 1);
    _hisvg_node_init (&filter->super.super, HISVG_NODE_TYPE_FILTER_PRIMITIVE_TILE, name);
    filter->super.in = g_string_new ("none");
    filter->super.result = g_string_new ("none");
    filter->super.x.factor = filter->super.y.factor = filter->super.width.factor =
        filter->super.height.factor = 'n';
    filter->super.render = hisvg_filter_primitive_tile_render;
    filter->super.super.free = hisvg_filter_primitive_free;
    filter->super.super.set_atts = hisvg_filter_primitive_tile_set_atts;
    return (HiSVGNode *) filter;
}
