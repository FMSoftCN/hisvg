...
typedef struct _HISVGParsePathCtx HISVGParsePathCtx;

struct _HISVGParsePathCtx {
...
static inline void
hisvg_path_builder_ensure_capacity (HiSVGPathBuilder *builder,
                                   int additional_capacity)
{
}

static inline void
hisvg_path_builder_add_element (HiSVGPathBuilder *builder,
...
void
hisvg_path_builder_init (HiSVGPathBuilder *builder,
                        int n_elements)
{
...
void
hisvg_path_builder_move_to (HiSVGPathBuilder *builder,
                           double x,
                           double y)
{
  cairo_path_data_t data;

  hisvg_path_builder_ensure_capacity (builder, 2);

  data.header.type = CAIRO_PATH_MOVE_TO;
  data.header.length = 2;
  hisvg_path_builder_add_element (builder, &data);
...
void
hisvg_path_builder_line_to (HiSVGPathBuilder *builder,
                           double x,
                           double y)
{
  cairo_path_data_t data;
...
void
hisvg_path_builder_curve_to (HiSVGPathBuilder *builder,
                            double x1,
                            double y1,
                            double x2,
                            double y2,
                            ...
                            double y3)
{
  ...
  hisvg_path_builder_ensure_capacity (builder, 4);
...
void
hisvg_path_builder_close_path (HiSVGPathBuilder *builder)
{
  cairo_path_data_t data;

  hisvg_path_builder_ensure_capacity (builder, 1);
  ...
  if (builder->last_move_to_index >= 0) {
    ...
    hisvg_path_builder_move_to (builder, moveto[1].point.x, moveto[1].point.y);
  }
}

cairo_path_t *
hisvg_path_builder_finish (HiSVGPathBuilder *builder)
{
    cairo_path_t *path;
...
static void
hisvg_path_arc_segment (HiSVGPathBuilder *builder,
                       double xc, double yc,
 ...
 *
 * Add an HISVG arc to the path context.
 **/
void
hisvg_path_builder_arc (HiSVGPathBuilder *builder,
                       double x1, double y1,
                       double rx, double ry,
                       double x_axis_rotation,
                       ...
                       double x2, double y2)
{
    ...
    for (i = 0; i < n_segs; i++)
        hisvg_path_arc_segment (builder, cx, cy,
...
static void
hisvg_parse_path_default_xy (HISVGParsePathCtx * ctx, int n_params)
{
    int i;
...
static void
hisvg_parse_path_do_cmd (HISVGParsePathCtx * ctx, gboolean final)
{
    double x1, y1, x2, y2, x3, y3;
    ...
    case 'm':
        ...
        if (ctx->param == 2 || final) {
            hisvg_parse_path_default_xy (ctx, 2);
            hisvg_path_builder_move_to (&ctx->builder, ctx->params[0], ctx->params[1]);
    ...
    case 'l':
        ...
        if (ctx->param == 2 || final) {
            hisvg_parse_path_default_xy (ctx, 2);
            hisvg_path_builder_line_to (&ctx->builder, ctx->params[0], ctx->params[1]);
            ctx->cp.point.x = ctx->rp.point.x = ctx->params[0];
            ctx->cp.point.y = ctx->rp.point.y = ctx->params[1];
            ctx->param = 0;
        }
    ...
    case 'c':
        ...
        if (ctx->param == 6 || final) {
            hisvg_parse_path_default_xy (ctx, 6);
            x1 = ctx->params[0];
            y1 = ctx->params[1];
            x2 = ctx->params[2];
            y2 = ctx->params[3];
            x3 = ctx->params[4];
            y3 = ctx->params[5];
            hisvg_path_builder_curve_to (&ctx->builder, x1, y1, x2, y2, x3, y3);
            ctx->rp.point.x = x2;
            ctx->rp.point.y = y2;
            ctx->cp.point.x = x3;
            ctx->cp.point.y = y3;
    ...
    case 's':
        ...
        if (ctx->param == 4 || final) {
            hisvg_parse_path_default_xy (ctx, 4);
    ...
    case 'h':
        /* horizontal lineto */
        if (ctx->param == 1) {
            hisvg_path_builder_line_to (&ctx->builder, ctx->params[0], ctx->cp.point.y);
            ctx->cp.point.x = ctx->rp.point.x = ctx->params[0];
    ...
    case 'v':
        /* vertical lineto */
        if (ctx->param == 1) {
            hisvg_path_builder_line_to (&ctx->builder, ctx->cp.point.x, ctx->params[0]);
    ...
    case 'a':
        if (ctx->param == 7 || final) {
            ...
            hisvg_path_builder_arc (&ctx->builder,
                                   x1, y1,
                                   rx, ry,
                                   x_axis_rotation,
                                   large_arc_flag,
...
static void
hisvg_path_end_of_number (HISVGParsePathCtx * ctx, double val, int sign, int exp_sign, int exp)
{
    ...
    hisvg_parse_path_do_cmd (ctx, FALSE);    
}

#define HISVGN_IN_PREINTEGER  0
#define HISVGN_IN_INTEGER     1
#define HISVGN_IN_FRACTION    2
#define HISVGN_IN_PREEXPONENT 3
#define HISVGN_IN_EXPONENT    4

#define HISVGN_GOT_SIGN          0x1
#define HISVGN_GOT_EXPONENT_SIGN 0x2

/* Returns the length of the number parsed, so it can be skipped
 * in hisvg_parse_path_data. Calls hisvg_path_end_number to have the number
 * processed in its command.
 */
static int
hisvg_parse_number (HISVGParsePathCtx * ctx, const char *data)
{
    int length = 0;
    int in = HISVGN_IN_PREINTEGER; /* Current location within the number */
    ...
    while (data[length] != '\0' && !end && !error) {
        char c = data[length];
        switch (in) {
            case HISVGN_IN_PREINTEGER: /* No numbers yet, we're just starting out */
                /* LEGAL: + - .->FRACTION DIGIT->INTEGER */
                if (c == '+' || c == '-') {
                    if (got & HISVGN_GOT_SIGN) {
                    ...
                    in = HISVGN_IN_FRACTION;
                } else if (c >= '0' && c <= '9') {
                    value = c - '0';
                    in = HISVGN_IN_INTEGER;
                }
                break;
            case HISVGN_IN_INTEGER: /* Previous character(s) was/were digit(s) */
                ...
                if (c >= '0' && c <= '9') {
                    ...
                    in = HISVGN_IN_PREEXPONENT;
                }
                else {
                    end = TRUE;
                }
                break;
            case HISVGN_IN_FRACTION: /* Previously, digit(s) in the fractional part */
                ...
                break;
            case HISVGN_IN_PREEXPONENT: /* Right after E */
                /* LEGAL: + - DIGIT->EXPONENT */
                if (c == '+' || c == '-') {
                    if (got & HISVGN_GOT_EXPONENT_SIGN) {
                        error = TRUE; /* Two signs: not allowed */
                    } else {
                        exponent_sign = c == '+' ? +1 : -1;
                        got |= HISVGN_GOT_EXPONENT_SIGN;
                    }
                } else if (c >= '0' && c <= '9') {
                    exponent = c - '0';
                    in = HISVGN_IN_EXPONENT;
                }
                break;
            case HISVGN_IN_EXPONENT: /* After E and the sign, if applicable */
                /* LEGAL: DIGIT */
                if (c >= '0' && c <= '9') {
    ...
    hisvg_path_end_of_number(ctx, value, sign, exponent_sign, exponent);
    return end /* && !error */ ? length - 1 : length;
}

static void
hisvg_parse_path_data (HISVGParsePathCtx * ctx, const char *data)
{
    int i = 0;
    char c = 0;
    ...
    for (i = 0; data[i] != '\0'; i++) {
        ...
        if ((c >= '0' && c <= '9') || c == '+' || c == '-' || c == '.') {
            /* digit */
            i += hisvg_parse_number(ctx, data + i) - 1;
        } else if (c == 'z' || c == 'Z') {
            if (ctx->param)
                hisvg_parse_path_do_cmd (ctx, TRUE);
            hisvg_path_builder_close_path (&ctx->builder);
...
cairo_path_t *
hisvg_parse_path (const char *path_str)
{
    HISVGParsePathCtx ctx;

    hisvg_path_builder_init (&ctx.builder, 32);
    ...
    hisvg_parse_path_data (&ctx, path_str);

    if (ctx.param)
        hisvg_parse_path_do_cmd (&ctx, TRUE);

    return hisvg_path_builder_finish (&ctx.builder);
}

void
hisvg_cairo_path_destroy (cairo_path_t *path)
{
    if (path == NULL)
        return;
